  ...
  static validateEnvironment(): void {
    ...
    if (aiProvider === 'azure') {
      ...
      if (!isNotEmptyString(azureApiKey)) {
        throw new Error(
          'Missing AZURE_OPENAI_API_KEY environment variable. Please provide a valid Azure OpenAI API key.',
        )
      }
      if (!isNotEmptyString(azureEndpoint)) {
        throw new Error(
          'Missing AZURE_OPENAI_ENDPOINT environment variable. Please provide your Azure OpenAI endpoint.',
        )
      }
