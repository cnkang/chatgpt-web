 ...
 */
export const FAST_PROPERTY_CONFIG: fc.Parameters<any> = {
  numRuns: 10,
  seed: 42,
  verbose: false,
}
...
export function propertyTest<T>(
  ...
  describe(`Property ${metadata.propertyNumber}: ${metadata.propertyTitle}`, () => {
    it(
      ...
      () => {
        fc.assert(fc.property(arbitrary, predicate) as any, {
          ...config,
...
export function asyncPropertyTest<T>(
  ...
  describe(`Property ${metadata.propertyNumber}: ${metadata.propertyTitle}`, () => {
    it(
      ...
      async () => {
        await fc.assert(fc.asyncProperty(arbitrary, predicate) as any, {
          ...config,
          examples: config.examples || [],
        })
      },
