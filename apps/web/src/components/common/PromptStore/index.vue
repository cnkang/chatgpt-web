...
const promptList = ref<Array<{ key: string; value: string }>>(promptStore.promptList)
...
const tempModifiedItem = ref<{ key: string; value: string }>({} as { key: string; value: string })
...
function modifyPromptTemplate() {
	...
	const tempList = promptList.value.filter(
